...
Since this is a barplot it is hard to see the trend between each age and average healthcare cost. However, by eye, we can see an overall positive trend if we compare the peaks of the bars for each age. In addition, the bar plot is useful as we are able to see extreme cases where the average cost increases by a lot or decreases by a lot. For example, from age 37 to age 38, the average cost drops by nearly $10,000. But because we aren't able to look into the samples for age 37, we don't know why the average medical cost for that group is so high.
...
From the two scatterplots above, we can see that between BMI and Charges, the smoker graph has a very strong positive correlation (0.806) with there being two major density 'blocks' seen.
...
# Summary/Conclusion 
After researching, it is clear that there are many factors of high healthcare charges. In addition, most of these factors are related to each other causing some factors to be more significant once analyzed against other factors. From the plots shown throughout, we can see that age is a significant factor towards high healthcare charges because as we get older, we are prone to more diseases. Whilst bmi becomes more significant once we separate it into smokers and non-smokers and thus showing that smokers are more likely exposed to health risks which causes healthcare charges to be higher than non-smokers. Furthermore, it is found that there is a positive linear relationship between Age and BMI and that sex, smoker's status, and region does affect the distribution of age in different BMI classifications. Through this project, we learnt to use pandas and data visualization tools to create insightful analysis.